...
func TestCostBasis(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected:     decimal.NewFromFloat(160002.50),
			expectError:  false,
		},
		{
...
func TestCostBasisPerUnit(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			quantity:     decimal.NewFromInt(2),
			pricePerUnit: decimal.NewFromFloat(80000.00),
			fee:          decimal.NewFromFloat(2.00),
			expected:     decimal.NewFromFloat(80001.00),
			expectError:  false,
		},
		{
			name:         "zero quantity returns error",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			assert.True(t, costBasisPerUnit.Amount.Equal(tt.expected), "expected %s, got %s", tt.expected, costBasisPerUnit.Amount)
		})
	}
}
